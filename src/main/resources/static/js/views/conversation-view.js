    ...
    var ConversationView = Backbone.View.extend({
        ...
        run: function () {
            ...
            $.ajax({
                ...
                error: function () {
                    var msg = arguments[0] ? arguments[0] : 'Check the URL.';
                    var initCount = 0;
                    $('#req-time').html('');
                    $('#status-code').html('FAIL');
                    $('#content-size').html('');
                    var iframe = document.getElementById('response-preview');
                    iframe = (iframe.contentWindow) ? iframe.contentWindow : (iframe.contentDocument.document) ? iframe.contentDocument.document : iframe.contentDocument;
                    iframe.document.open();
                    $("#response-wrapper").html('<br><pre class="prettyprint"> ' + msg + ' </pre>');
                    iframe.document.close();

                    // remove assertResult
                    var spans = $('#res-tab-assert').find('span');
                    $(spans[0]).html(initCount);
                    $(spans[1]).html(initCount);
                    $('#res-assert-wrapper tbody').empty();
                    $('#res-header-wrapper tbody').empty();
                    $('#loadingRequest').hide();
                },
